...
class PitchingBeep{
    ...
    var diffTargetAngle: Double = 2.0
    ...
    private func handleDataPairChange(_ carv2DataPair: Carv2AnalyzedDataPair) {
        ...
        if (-diffTargetAngle...diffTargetAngle).contains(Double(carv2DataPair.pitchingAngulerRateDiffrential) ) {
            conductor.data.isPlaying = false
        } else {
            conductor.data.isPlaying = true
        }
        if carv2DataPair.yawingAngulerRateDiffrential > 0 {
            conductor.data.frequency = AUValue(hight(ceil(carv2DataPair.pitchingAngulerRateDiffrential * 10)))
            conductor.changeWaveFormToSin()
        } else {
            conductor.changeWaveFormToTriangle()
            conductor.data.frequency = AUValue(lowToHigh(-ceil(carv2DataPair.pitchingAngulerRateDiffrential * 10)))
        }
    }
